...
export function CatalogMenu({ items, storeName = "Cannabis Menu", logo, brandColors = [], onItemUpdate, onItemDelete, categoryOrder: propCategoryOrder, categorySettings, repeatHeader }: MenuProps) {
  ...
  const renderHeader = (isRepeat = false) => (
    <header className={`catalog-header ${isRepeat ? "catalog-header-repeat" : ""}`}>
      <div className="catalog-header-content">
        {logo && <img src={logo} alt="Logo" className="catalog-logo" />}
        <div className="catalog-header-text">
          <h1 className="catalog-title">{storeName}</h1>
          <p className="catalog-subtitle">Product Catalog</p>
        </div>
      </div>
      <div className="catalog-header-line" style={{ background: `hsl(${primary.h} ${primary.s}% ${primary.l}%)` }}></div>
    </header>
  );
  ...
  return (
    <article
      id="printable-menu"
      className="print-menu catalog-menu"
      style={{
        // @ts-ignore
    ...
    >
      ...
      {/* Categories - each takes full width */}
        ...
        {categoryOrder.map((category, catIndex) => {
          ...
          return itemChunks.map((chunkItemsList, chunkIndex) => {
            ...
            <section
              ...
              className={`catalog-category ${hasPageBreak ? "page-break-before" : ""}`}
            >
              ...
              <div className="catalog-category-header">
                <h2 className="catalog-category-title" style={{ color: `hsl(${primary.h} ${primary.s}% ${primary.l}%)` }}>
                  {category}{itemChunks.length > 1 ? ` (${chunkIndex + 1}/${itemChunks.length})` : ""}
                </h2>
                <div className="catalog-category-line" style={{ background: `hsl(${primary.h} ${primary.s}% ${primary.l}% / 0.3)` }}></div>
              </div>
                  ...
                  <div key={item.id} className="catalog-item">
                    {onItemDelete && (
                      <button
                        ...
                        className="catalog-item-delete no-print"
                        title="Remove item"
                      >
                        <X className="w-3 h-3" />
                      </button>
                    ...
                    <div className="catalog-item-details">
                      ...
                      {item.strain && (
                        <EditableField
                          className="catalog-detail catalog-strain"
                    ...
                    {(item.thc || item.cbd) && (
                      <div className="catalog-item-lab">
                        {item.thc && (
                          <EditableField
                            className="catalog-thc"
                            value={item.thc}
                            itemId={item.id}
                            field="thc"
                            onUpdate={onItemUpdate}
